...
if __name__=="__main__":
    ...
    s3_output_all_file_name = 'all_results.csv'
    s3_output_best_file_name ='best_results.csv'
    s3_training_history_file = 'training_history.csv'

    output_all_file_name = './' + s3_output_all_file_name
    output_best_file_name = './' + s3_output_best_file_name
    training_history_file = './' + s3_training_history_file

    s3_info_file_name = "locality_info.csv"
    s3_limits_file_name = "lice_limits.csv"

    info_file_name = './' + s3_info_file_name
    limits_file_name = './' + s3_limits_file_name

    s3_avgFL_file_name = "final_avgFL.csv"
    s3_temperature_file_name = "final_temperature.csv"
    s3_treatment_file_name = "final_treatment.csv"
    s3_liceType_file_name = "final_LiceType.csv"

    avgFL_file_name = './' + s3_avgFL_file_name
    temperature_file_name = './' + s3_temperature_file_name
    treatment_file_name = './' + s3_treatment_file_name
    liceType_file_name = './' + s3_liceType_file_name
    ...
    s3.download_file(bucket_name, s3_info_file_name, info_file_name)
    s3.download_file(bucket_name, s3_limits_file_name, limits_file_name)
    s3.download_file(bucket_name, s3_avgFL_file_name, avgFL_file_name)
    s3.download_file(bucket_name, s3_temperature_file_name, temperature_file_name)
    s3.download_file(bucket_name, s3_treatment_file_name, treatment_file_name)
    s3.download_file(bucket_name, s3_liceType_file_name, liceType_file_name)
    ...
    try:
        s3.delete_object(Bucket=bucket_name, Key=s3_output_all_file_name)
    except:
        print(" * Unable to delete All results file from S3 * ")

    try:
        s3.delete_object(Bucket=bucket_name, Key=s3_output_best_file_name)
    except:
        print(" * Unable to delete Best results file from S3 * ")

    try:
        s3.delete_object(Bucket=bucket_name, Key=s3_training_history_file)
    except:
    ...
    s3.upload_file(avgFL_file_name, bucket_name, s3_avgFL_file_name)
